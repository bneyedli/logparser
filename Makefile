...
build:
	...
	@docker build . --build-arg LOG_DIR=$(CONTAINER_LOG_DIR)/$(CONTAINER_LOG_FILE) --build-arg LISTEN_PORT=$(LISTEN_PORT) -t $(IMAGE_NAME)
run: $(HOST_LOG_DIR)
	@echo "Running container: $(IMAGE_NAME) Listen IP: $(LISTEN_IP) Log source: $(HOST_LOG_DIR)"
	@docker run -it --mount type=bind,source="$(HOST_LOG_DIR)",target=$(CONTAINER_LOG_DIR) --env "LISTEN_IP=$(LISTEN_IP)" --env "LISTEN_PORT=$(LISTEN_PORT)" --env "LOG_FILE=$(CONTAINER_LOG_DIR)/$(CONTAINER_LOG_FILE)" $(IMAGE_NAME)
